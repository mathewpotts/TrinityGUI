...
LWXDIR = LHOMEDIR + "weather/" # director for the path of the weather data
...
BINDIR    = HOMEDIR + "/bin/"
CAMDIR    = RESDIR + "cams/"
WXDIR     = RESDIR + "weather/"
WPDIR     = RESDIR + "weather_plots/"

refresh_rate = 60000 # milliseconds # the speed of refreshing the data
########## End of File paths ##########################
#######################################################
########## Login Information ##########################
#######################################################
if os.path.isfile(RESDIR + "/tmp.txt"):
    ...
    PHYS_USR  = lines[2].split()[1]
    PHYS_PASS = lines[2].split()[2]
    ...
    PHYS_USR  = input(f"{PHYS_HOST} Username: ")
    PHYS_PASS = input(f"{PHYS_HOST} Password: ")
    PORT      = input("Input Tunnel Port: ")
    CTRL_PASS = input("Control PC Password: ")
    print(f'{PORT}\n{GT_HOST} {GT_USR} {GT_PASS}\n{PHYS_HOST} {PHYS_USR} {PHYS_PASS}\n{CTRL_PASS}',
          file=open(RESDIR + '/tmp.txt','w'))
    ...
    def __init__(self, parent, controller):
        ...
        def inside_images(file_path_in):
            ...
            try:
                ...
                ttk.Label(self, text='Indoor Camera', font=SmFONT).place(x=350,y=400)  # make a label of the file name
                # create the button holder
                inside_but = tk.Button(self, text='Click Me !', image=self.image_in)
                inside_but.place(x=350, y=25)  # set the location
                ttk.Label(self, text='Indoor Camera', font=SmFONT).place(x=350,y=400)  # make a label of the file name
        ...
        def outside_images(file_path_out):
            ...
            subprocess.run(cmd.split())
            file_out = f'{CAMDIR}/OUTCAM.jpg'
            try:
                # resize the image
                img_out = Image.open(file_out).resize((550, 350))
                ...
                ttk.Label(self, text='Outdoor Camera', font=SmFONT).place(x=950, y=400)
            except:
                print('OSError: image file is truncated.')
        ...
        def make_plots_and_labels(file):
            ...
            def save_plot_png(all_data):
                ...
                def plot_format(x, y, xlabel, ylabel, title, color, fig, thick=1):
                    ...
                    plt.savefig(f'{WPDIR}/{title}.png')  # save the image as a png
                    pickle.dump(fig, open(f'{WPDIR}/{title}.pickle','wb'))  # save the image as a pickle so that it can be view in python again
...
Astro Twilight:                 {astro_twi}Z\n''' 
                ...
                plt.axis('off')
                plt.savefig(f"{RESDIR}/wind_label_readout.png")
                plt.close()
            ...
            def label_image():
                ...
                img_label = Image.open(f"{RESDIR}/wind_label_readout.png")
...
def create_tunnel():
    ...
    ssh_cmd = f"ssh -X -o StrictHostKeyChecking=no {GT_USR}@{GT_HOST} -L {PORT}:{PHYS_HOST}:22"
    print(ssh_cmd)
    try:
